...
class Ivole_Reviews_Admin_Menu {
    ...
    public function publish_reply_to_CR_checkbox( $name ) {
      ...
      if ( 'yes' === get_option( 'ivole_reviews_verified', 'no' ) ) {
        ...
        if( strlen( $licenseKey ) > 0 ) {
          echo '<input type="checkbox" id="' . $name . '_checkbox" name="' . $name . '" value="yes" checked="checked" />';
          echo '<label for="' . $name . '_checkbox">' . _e( 'Publish a copy of your reply to CR', IVOLE_TEXT_DOMAIN ) . '</label>';
        } else {
          echo _e( sprintf( 'A copy of this reply will not be published on <a href="https://www.cusrev.com/reviews/' . $this->ivole_verified_page . '" target="_blank">CR (Customer Reviews) portal</a> because no license key was entered in the <a href="%s">plugin\'s settings</a>. A license key (free or premium) is used as a secret token for additional security when communicating with CR service.', admin_url( 'admin.php?page=ivole-reviews-settings&tab=license-key' ) ), IVOLE_TEXT_DOMAIN );
        }
      }
      else {
        echo _e( sprintf( 'A copy of this reply will not be published on CR (Customer Reviews) portal because Verified Reviews Page option is <a href="%s">disabled</a>.', admin_url( 'admin.php?page=ivole-reviews-settings&tab=trust_badges' ) ), IVOLE_TEXT_DOMAIN );
      }
    }
        ...
        //if the reply was published to CR, add a label indicating this
          ...
          wp_nonce_field( 'ivole_update_reply_cr', 'ivole_update_reply_cr_checkbox' );
            ...
            $order_product = Ivole_Replies::isReplyForCRReview( $comment );
  					echo '<p class="ivole-reply-published-cr-c"><span class="dashicons dashicons-yes"></span> ' . __( 'A copy of this reply was published on <a href="https://www.cusrev.com/reviews/' . $this->ivole_verified_page . '/p/p-' . $order_product[1] . '/r-' . $order_product[0] . '" target="_blank">CR</a><span class="dashicons dashicons-external"></span>', IVOLE_TEXT_DOMAIN ) . '</span></p>';
  				}
  			} else {
    ...
    public function update_cr_review( $comment_ID, $data ) {
      ...
      } else if( isset( $_POST['ivole_update_reply_cr_checkbox'] ) && wp_verify_nonce( $_POST['ivole_update_reply_cr_checkbox'], 'ivole_update_reply_cr' ) ) {
        $cr_reply = new Ivole_Replies( $comment_ID );
      }
    }
